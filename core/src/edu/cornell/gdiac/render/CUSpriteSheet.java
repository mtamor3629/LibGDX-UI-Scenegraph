 ...
 * CUSpriteSheet.java
 *
 * A sprite sheet is a single image with multiple copies of the game object
 ...
 * they are at the end of the sprite sheet (e.g. the bottom right).
 * 
 ...
 */
public class CUSpriteSheet extends TextureRegion {
	 ...
	 */
	public CUSpriteSheet(Texture texture, int rows, int cols) {
	 ...
	 */
	public CUSpriteSheet(Texture texture, int rows, int cols, int size) {
		super(texture);
		if (size > rows*cols) {
	 ...
	 */
	public CUSpriteSheet copy() {
		return new CUSpriteSheet( getTexture(), rows, cols, size );
	}
